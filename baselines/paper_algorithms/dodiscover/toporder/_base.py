...
class CAMPruning:
    ...
    def _fit_gam_model(self, X: NDArray, y: NDArray) -> LinearGAM:
        ...
        """
        lambda_candidates = {1: [0.1, 0.5, 1], 20: [5, 10, 20], 100: [50, 80, 100]}
        ...
        # Fit with the selected lambda candidates.
        lam = np.squeeze([lambda_candidates[value] for value in lambdas])
        gam = LinearGAM(formula, fit_intercept=False).gridsearch(
