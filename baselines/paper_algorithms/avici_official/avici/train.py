...
def retrieve_ave(ave_d):
    ...
    for k, v in ave_d.items():
        ...
        # try flatten any tensor-like value
        try:
            ...
            try:
                ...
                # last fallback
                try:
                    v_val = float(np.asarray(v).reshape(-1)[0])
                except Exception:
                    v_val = float(np.asarray(v).ravel()[0])

        ctr = np.asarray(ave_d[("__ctr__", k)]).astype(float).reshape(-1)[0]
        out[k] = v_val / ctr
    ...
    for descr, test_iters in all_test_iters.items():
        for n_vars, it in test_iters.items():
            ...
            for n, scalars_ave in scalars_ave_stratified.items():
                df = pd.concat([df, pd.DataFrame([{ **scalars_ave, "descr": descr, "d": n_vars, "n": n, "eval_time": eval_time }])], ignore_index=True)
