...
class OrderFriendlyPath(GraphModel):
    """
    Order-friendly DAG made of many short directed chains.

    A single length-100 chain can create very deep causal cascades and make the
    downstream SCM numerically unstable. Short chains still provide clean
    length-3..6 order supervision while keeping the SEM depth bounded.
    """
    order_friendly = True

    def __init__(self, shortcut_prob=0.05, max_skip=3, chain_length=6, max_edges_per_var=1.5):
        self.shortcut_prob = shortcut_prob
        self.max_skip = max_skip
        self.chain_length = chain_length
        self.max_edges_per_var = max_edges_per_var

    def __call__(self, rng, n_vars):
        mat_ordered = onp.zeros((n_vars, n_vars), dtype=int)
        if n_vars <= 1:
            return mat_ordered

        chain_length = max(3, int(self.chain_length))
        max_edges = max(1, int(round(float(self.max_edges_per_var) * n_vars)))

        # Build disjoint short chains: enough local order paths, no long depth.
        for start in range(0, n_vars, chain_length):
            end = min(n_vars, start + chain_length)
            for i in range(start, end - 1):
                mat_ordered[i, i + 1] = 1

        max_skip = max(2, int(self.max_skip))
        optional_edges = []
        for start in range(0, n_vars, chain_length):
            end = min(n_vars, start + chain_length)
            for i in range(start, end):
                upper = min(end, i + max_skip + 1)
                for j in range(i + 2, upper):
                    optional_edges.append((i, j))

        rng.shuffle(optional_edges)
        for i, j in optional_edges:
            if mat_ordered.sum() >= max_edges:
                break
            if rng.uniform() < self.shortcut_prob:
                mat_ordered[i, j] = 1

        p = rng.permutation(onp.eye(n_vars).astype(int))
        mat = p.T @ mat_ordered @ p
        assert ig.Graph.Weighted_Adjacency(mat.tolist()).is_dag()
        return mat


class OrderFriendlyLayered(GraphModel):
    """
    Sparse adjacent-layer DAG with a clear topological direction.

    Edges are restricted to adjacent layers and the total edge count is capped.
    This gives source/sink order supervision without creating dense high-depth
    mechanisms that often generate NaN/Inf samples.
    """
    order_friendly = True

    def __init__(self, n_layers=8, edge_prob=0.08, bridge_prob=0.8, max_edges_per_var=1.5):
    ...
    def __call__(self, rng, n_vars):
        mat = onp.zeros((n_vars, n_vars), dtype=int)
        if n_vars <= 1:
            return mat

        n_layers = max(3, min(int(self.n_layers), n_vars))
        order = rng.permutation(n_vars)
        splits = onp.array_split(order, n_layers)
        max_edges = max(1, int(round(float(self.max_edges_per_var) * n_vars)))

        optional_edges = []
        for li in range(n_layers - 1):
            for src in splits[li]:
                for tgt in splits[li + 1]:
                    optional_edges.append((int(src), int(tgt)))

        rng.shuffle(optional_edges)
        for src, tgt in optional_edges:
            if mat.sum() >= max_edges:
                break
            if rng.uniform() < self.edge_prob:
                mat[src, tgt] = 1

        # Add at most one bridge per adjacent layer pair, preserving sparsity.
        for li in range(n_layers - 1):
            if mat.sum() >= max_edges:
                break
            if len(splits[li]) > 0 and len(splits[li + 1]) > 0 and rng.uniform() < self.bridge_prob:
                src = int(rng.choice(splits[li]))
                tgt = int(rng.choice(splits[li + 1]))
                mat[src, tgt] = 1
...
class OrderFriendlyBanded(GraphModel):
    """
    Sparse local-band DAG.

    Local forward edges create many short order paths. Edge count and chain
    depth are bounded to keep generated SCM samples numerically safe.
    """
    order_friendly = True

    def __init__(self, band=3, edge_prob=0.10, backbone_prob=0.35, max_edges_per_var=1.5):
        self.band = band
        self.edge_prob = edge_prob
        self.backbone_prob = backbone_prob
        self.max_edges_per_var = max_edges_per_var

    def __call__(self, rng, n_vars):
        mat_ordered = onp.zeros((n_vars, n_vars), dtype=int)
        band = max(1, int(self.band))
        max_edges = max(1, int(round(float(self.max_edges_per_var) * n_vars)))

        optional_edges = []
        for i in range(n_vars):
            upper = min(n_vars, i + band + 1)
            for j in range(i + 2, upper):
                optional_edges.append((i, j))

        # Light local backbone, but not a full-depth chain.
        for i in range(n_vars - 1):
            if mat_ordered.sum() >= max_edges:
                break
            if rng.uniform() < self.backbone_prob:
                mat_ordered[i, i + 1] = 1
...
class ScreeningFlow(GraphModel):
    """
    Sparse funnel-like DAG for screening / escalation workflows.

    The graph is organized into ordered stages with mostly adjacent-stage edges,
    plus a few safe skip edges. This creates realistic screening chains without
    degenerating into extremely dense layered graphs.
    """
    def __init__(self, n_stages=5, edge_prob=0.18, skip_prob=0.05, max_edges_per_var=1.4):
    ...
    def __call__(self, rng, n_vars):
        ...
        n_stages = max(4, min(int(self.n_stages), n_vars))
        ordered_nodes = rng.permutation(n_vars)
        stages = onp.array_split(ordered_nodes, n_stages)
        max_edges = max(1, int(round(float(self.max_edges_per_var) * n_vars)))

        # Adjacent-stage candidate edges.
        candidates = []
        for si in range(n_stages - 1):
            src_stage = stages[si]
            tgt_stage = stages[si + 1]
            for src in src_stage:
                for tgt in tgt_stage:
                    candidates.append((int(src), int(tgt), self.edge_prob))

        # A small number of skip edges: screen -> follow-up, triage -> action.
        for si in range(n_stages - 2):
            src_stage = stages[si]
            tgt_stage = stages[si + 2]
            for src in src_stage:
                for tgt in tgt_stage:
                    candidates.append((int(src), int(tgt), self.skip_prob))

        rng.shuffle(candidates)
        for src, tgt, prob in candidates:
            if mat.sum() >= max_edges:
                break
            if rng.uniform() < prob:
                mat[src, tgt] = 1

        # Guarantee every non-root stage node has at least one parent.
        for si in range(1, n_stages):
            parent_pool = onp.concatenate(stages[:si])
            for tgt in stages[si]:
                tgt = int(tgt)
                if mat[:, tgt].sum() == 0 and len(parent_pool) > 0 and mat.sum() < max_edges:
                    src = int(rng.choice(parent_pool))
                    mat[src, tgt] = 1
...
class RiskRootFork(GraphModel):
    """
    DAG with a few upstream risk roots and structured local fan-out.

    This prior is meant to teach that high-level risk sources affect many, but
    not all, downstream nodes. Each root owns local modules, which creates
    hard negatives around "almost connected" non-edges without making the graph
    fully hub-dominated.
    """
    def __init__(self, n_roots=2, branch_prob=0.55, local_chain_prob=0.45,
                 overlap_prob=0.08, max_edges_per_var=1.6):
        self.n_roots = n_roots
        self.branch_prob = branch_prob
        self.local_chain_prob = local_chain_prob
    ...
    def __call__(self, rng, n_vars):
        ...
        order = rng.permutation(n_vars)
        n_roots = max(1, min(int(self.n_roots), max(1, n_vars // 3)))
        roots = order[:n_roots]
        remainder = order[n_roots:]
        max_edges = max(1, int(round(float(self.max_edges_per_var) * n_vars)))

        if len(remainder) == 0:
            return mat

        modules = [arr.tolist() for arr in onp.array_split(remainder, n_roots)]
        for ridx, root in enumerate(roots):
            ...
            branch_targets = []
            for node in module:
                if rng.uniform() < self.branch_prob:
                    branch_targets.append(int(node))

            if not branch_targets:
                branch_targets.append(int(module[0]))
            ...
            for i in range(len(module) - 1):
                if mat.sum() >= max_edges:
                    break
                if rng.uniform() < self.local_chain_prob:
                    mat[int(module[i]), int(module[i + 1])] = 1

            # Sparse cross-root overlap to keep roots correlated but not universal.
            for other_root in roots:
                if other_root == root or mat.sum() >= max_edges:
                    continue
                for tgt in module:
                    if rng.uniform() < self.overlap_prob:
                        mat[int(other_root), int(tgt)] = 1
                        break
