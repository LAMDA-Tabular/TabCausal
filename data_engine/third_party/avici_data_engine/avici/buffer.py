...
class Sampler:
    ...
    def generate_data(rng, *, n_vars, spec_list, spec=None):
        ...
        collection = dict(
            ...
            order_friendly=onp.int32(1 if getattr(spec.graph, "order_friendly", False) else 0),
        )
    ...
    def async_worker(j, state, worker_seed, it, descr, n_vars, config):
        ...
        if plasma is None:
            raise ImportError("pyarrow.plasma is required for asynchronous buffering.")
